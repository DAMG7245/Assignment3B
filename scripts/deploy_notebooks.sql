...
CREATE OR REPLACE NOTEBOOK IDENTIFIER('"CRYPTO_DB"."{{env}}_SCHEMA"."{{env}}_Data_Aggregation_and_Analytics"')
    FROM '@"CRYPTO_DB"."INTEGRATIONS"."CRYPTO_GIT_REPO"/branches/"{{branch}}"/notebooks/04.Data_Aggregation_and_Analytics/'
    QUERY_WAREHOUSE = 'CRYPTO_WH'
    MAIN_FILE = '04.Data_Aggregation_and_Analytics.ipynb';

-- Add a live version
ALTER NOTEBOOK "CRYPTO_DB"."{{env}}_SCHEMA"."{{env}}_Data_Aggregation_and_Analytics" ADD LIVE VERSION FROM LAST;

CREATE OR REPLACE NOTEBOOK IDENTIFIER('"CRYPTO_DB"."{{env}}_SCHEMA"."{{env}}_Task_Orchestration_and_Automation"')
    FROM '@"CRYPTO_DB"."INTEGRATIONS"."CRYPTO_GIT_REPO"/branches/"{{branch}}"/notebooks/05.Task_Orchestration_and_Automation/'
    QUERY_WAREHOUSE = 'CRYPTO_WH'
    MAIN_FILE = '05_Task_Orchestration_and_Automation.ipynb';

-- Add a live version
ALTER NOTEBOOK "CRYPTO_DB"."{{env}}_SCHEMA"."{{env}}_Task_Orchestration_and_Automation" ADD LIVE VERSION FROM LAST;
